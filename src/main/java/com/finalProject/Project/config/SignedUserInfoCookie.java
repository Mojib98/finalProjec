...
public class SignedUserInfoCookie extends Cookie {
    ...
    public SignedUserInfoCookie(Cookie cookie, String cookieHmacKey) {
        ...
        if (hmac == null)
            throw new IllegalStateException("Cookie not signed (no HMAC)");
        ...
        if (!hmac.equals(calculateHmac(payload, cookieHmacKey)))
            throw new IllegalStateException("Cookie signature (HMAC) invalid");
    ...
    public User getUserInfo() {
        return new User(
                payload.username,
                payload.roles.stream().map(SimpleGrantedAuthority::new).toString()) {
        };
    }
